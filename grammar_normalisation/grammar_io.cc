...
void GrammarIO::Read(int& nonterminals_cnt, int& terminals_cnt,
                     int& start_symbol, std::vector<Production>& productions, SymbolTable &symbol_table) {
...
bool GrammarIO::ReadProduction(std::vector<Production> &productions, SymbolTable &symbol_table) {
    std::string production_error = "Invalid format of production: ";

    std::vector<std::string> line = GetTokenizedLine();
    if (line.size() == 0) return false;
    if (line.size() < 3 || !symbol_table.IsNonterminal(line[0]) || line[1] != "->") {
        Error(production_error+"Production should consist of nonterminal symbol, arrow and positive number of conjuncts.");
    }
    // if (line.size() == 3) {
    ...
    while (it < line.size()) {
        ...
        while (it < (int)line.size() && line[it] != "|") {
            ...
            std::string symbol;
            while (it < (int)line.size() && ((symbol = line[it]) != "&") && (symbol != "|")) {
                if (symbol_table.IsNonterminal(symbol)) {
                    conjunct.push_back(Symbol(SymbolType::kNonterminal,symbol_table.GetNonterminalId(symbol)));
                } 
                else if (symbol_table.IsTerminal(symbol)) {
                    conjunct.push_back(Symbol(SymbolType::kTerminal,symbol_table.GetTerminalId(symbol)));
                }
            ...
            if (conjunct.size() > 1) {
                for (Symbol symbol: conjunct) {
                    if (symbol.type == SymbolType::kEpsilon) {
                        Error("Conjunct must be either epsilon or (nonterminals + terminals)+");
                    }
                }
            }
            if (conjunct.empty()) {
...
void GrammarIO::ReadNonterminals(int& nonterminals_cnt, int &start_symbol, SymbolTable &symbol_table) {
    std::vector<std::string> line = GetTokenizedLine();
    if (line.size() == 0) {
...
void GrammarIO::PrintProductions(std::vector<Production> &productions, SymbolTable &symbol_table) {
