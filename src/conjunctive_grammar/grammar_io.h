...
class GrammarIO {
 public:
  GrammarIO();
  bool Read(std::istream &input, int &start_symbol,
            std::vector<Production> &productions, SymbolTable &symbol_table);
  void Print(std::ostream &output, int &start_symbol,
             std::vector<Production> &productions, SymbolTable &symbol_table);
 ...
 private:
  ...
  int ReadNonterminals(std::istream &input, int &start_symbol,
                       SymbolTable &symbol_table);
  int ReadTerminals(std::istream &input, SymbolTable &symbol_table);
  void PrintProductions(std::ostream &output,
                        std::vector<Production> &productions,
                        SymbolTable &symbol_table);
  void PrintTerminals(std::ostream &output, SymbolTable &symbol_table);
  void PrintNonterminals(std::ostream &output, int &start_symbol,
                         SymbolTable &symbol_table);
};
}  // namespace conjunctive_grammar
